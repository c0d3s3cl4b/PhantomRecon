...
def parse_ports(spec: str | None) -> list[int]:
    """Parse a comma-separated list and ranges into validated TCP ports."""
    if not spec:
    ...
    if len(ports) > MAX_PORTS_PER_SCAN:
        raise ValueError(
            f"A single scan is limited to {MAX_PORTS_PER_SCAN} ports"
        )
    return sorted(ports)


def normalize_target(target: str) -> tuple[str, str]:
    """Resolve a validated IP/domain target."""
...
def scan_port(
    ...
    """Check whether one TCP port accepts a connection."""
    try:
...
def scan_ports(
    ...
    *,
    ...
    """Perform a bounded TCP connect scan against an authorized target."""
    try:
        ip, original = normalize_target(target)
    except ValueError as exc:
        return ScanResult.failure("port_scanner", target, str(exc))
...
def run() -> None:
    """Run the classic interactive port scanner interface."""
    ...
    console.print(
        f"[green]Open ports:[/green] "
        f"{result.data['open_count']} / {result.data['ports_scanned']}"
    )
    for item in result.data["open_ports"]:
        console.print(f"  {item['port']}/tcp  {item['service']}")

    report = {
        "Target": result.target,
        "Resolved IP": result.data["resolved_ip"],
        "Ports Scanned": result.data["ports_scanned"],
        "Open Ports": result.data["open_count"],
    }
    for item in result.data["open_ports"]:
        report[f"Port {item['port']}"] = item["service"]

    ask_save_report(report, "port_scanner", result.target)
    pause()
