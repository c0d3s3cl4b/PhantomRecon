...
SUBDOMAIN_WORDLIST = ["www", "mail", "api", "dev", "staging", "test", "app", "blog", "shop", "portal", "cdn", "static", "assets", "auth", "sso", "docs", "status"]
...
def crt_sh_lookup(domain: str, timeout: float = 15.0) -> set[str]:
    found: set[str] = set()
    try:
        response = requests.get("https://crt.sh/", params={"q": f"%.{domain}", "output": "json"}, timeout=timeout)
...
def find_subdomains(domain: str, *, active_dns: bool = False, timeout: float = 15.0, workers: int = 10) -> ScanResult:
    try:
        domain = normalize_domain(domain)
    except ValueError as exc:
        return ScanResult.failure("subdomain_finder", domain, str(exc))
    records: dict[str, dict[str, object]] = {name: {"hostname": name, "sources": ["crt.sh"], "addresses": []} for name in crt_sh_lookup(domain, timeout)}
    if active_dns:
        workers = max(1, min(workers, 20))
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(resolve_subdomain, f"{word}.{domain}"): f"{word}.{domain}" for word in SUBDOMAIN_WORDLIST}
            for future in as_completed(futures):
                hostname = futures[future]
                addresses = future.result()
                if addresses:
                    record = records.setdefault(hostname, {"hostname": hostname, "sources": [], "addresses": []})
    ...
    return ScanResult(module="subdomain_finder", target=domain, data={"domain": domain, "mode": "passive+dns" if active_dns else "passive", "total": len(subdomains), "subdomains": subdomains})
...
def run() -> None:
    ...
    if not target:
        print_error("No domain entered."); pause(); return
    result = find_subdomains(target)
    if result.status != "success":
        print_error(result.errors[0]); pause(); return
    entries = result.data["subdomains"]
    if entries:
        table = Table(title=f"[bold green]Subdomains Found ({len(entries)})[/bold green]", box=box.ROUNDED, border_style="green")
        table.add_column("Subdomain"); table.add_column("Source")
